CREATE OR REPLACE TYPE api_core."PDA_SHOPIDENT_TYPE"                                          FORCE AS OBJECT
-- ***************************************************************************
--  TYPE        : API_CORE.PDA_SHOPIDENT_TYPE
--  DESCRIPTION : Description de l'objet type SHOPIDENT
-- ---------------------------------------------------------------------------
--          | DATE       | AUTHOR
--          | DESCRIPTION
-- ---------------------------------------------------------------------------
...
, TAB_PDA_SHOPIDENT   api_core.TAB_COMMON_SHOPIDENT_TYPE
, CONSTRUCTOR FUNCTION PDA_SHOPIDENT_TYPE(SELF IN OUT NOCOPY PDA_SHOPIDENT_TYPE) RETURN SELF AS RESULT
...
/
CREATE OR REPLACE TYPE BODY api_core."PDA_SHOPIDENT_TYPE" 
-- ***************************************************************************
--  TYPE        : API_CORE.PDA_SHOPIDENT_TYPE
--  DESCRIPTION : Description de l'objet type SHOPIDENT
-- ---------------------------------------------------------------------------
...
IS
BEGIN
   SELF := PDA_SHOPIDENT_TYPE
      ( FILE_PDA_ID        => NULL
      , FILE_PDA_BUILD     => NULL
      , FILE_VERSION       => NULL
      , FILE_DTM           => NULL
      , TAB_PDA_SHOPIDENT  => NULL
      );
...
BEGIN
   ...
   IF (TAB_PDA_SHOPIDENT) IS NULL THEN
      l_result:= PCK_API_TOOLS.LIST(P_LIST => l_result, p_item => 'TAB_PDA_SHOPIDENT');
   END IF;
